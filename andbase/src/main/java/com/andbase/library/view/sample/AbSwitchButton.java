...
public class AbSwitchButton extends CompoundButton {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.AbSwitchButton);
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.AbSwitchButton_thumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.AbSwitchButton_thumbColor);
            margin = ta.getDimension(R.styleable.AbSwitchButton_thumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.AbSwitchButton_thumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.AbSwitchButton_thumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.AbSwitchButton_thumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.AbSwitchButton_thumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.AbSwitchButton_thumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.AbSwitchButton_thumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.AbSwitchButton_thumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.AbSwitchButton_backRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.AbSwitchButton_backDrawable);
            backColor = ta.getColorStateList(R.styleable.AbSwitchButton_backColor);
            backMeasureRatio = ta.getFloat(R.styleable.AbSwitchButton_backMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.AbSwitchButton_animationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.AbSwitchButton_fadeBack, true);
            tintColor = ta.getColor(R.styleable.AbSwitchButton_tintColor, tintColor);
            textOn = ta.getString(R.styleable.AbSwitchButton_textOn);
            textOff = ta.getString(R.styleable.AbSwitchButton_textOff);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.AbSwitchButton_textMarginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.AbSwitchButton_autoAdjustTextPosition, autoAdjustTextPosition);
            ta.recycle();
        }
        ...
        if (mTintColor == 0) {
            mTintColor = AbColorUtil.getAttrColor(context,R.attr.colorAccent);
        }
        ...
        if (!mIsBackUseDrawable && mBackColor == null) {
            mBackColor = AbColorUtil.generateBackColorWithTintColor(mTintColor);
