...
public class TileDrawable extends Drawable {
    ...
    private void initTiles(Point container) {
        if (mTiles == null) {
            ...
            AbPhotoImageView.MathUtils.calculateScaledRectInContainer(new RectF(0, 0, sWidth, sHeight), iWidth, iHeight, ImageView.ScaleType.FIT_CENTER, scaledRect);
    ...
    private Handler mRequestCurrentTilesDelayHandler = new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            if (getCallback() != null && getCallback() instanceof AbPhotoImageView) {
                requestCurrentTiles((AbPhotoImageView) getCallback());
    ...
    private void requestCurrentTiles(AbPhotoImageView pinchImageView) {
        if (mTiles != null && pinchImageView.getWidth() > 0 && pinchImageView.getHeight() > 0) {
            ...
            float scale = AbPhotoImageView.MathUtils.getMatrixScale(matrix)[0];
            ...
            for (int i = mTiles.length - 1; i >= 0; i--) {
                for (Tile tile : mTiles[i]) {
                    if (tile.mStatus == Tile.STATUS_LOADED && tile.mBitmap != null) {
                        ...
                        AbPhotoImageView.MathUtils.calculateRectTranslateMatrix(new RectF(0, 0, tile.mBitmap.getWidth(), tile.mBitmap.getHeight()), new RectF(tile.mSampleRect), matrix);
